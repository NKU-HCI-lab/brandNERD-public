...
const FILE_SIMILAR_ONEWAY=`${FOLDER_SIMILARITY}/similar_oneway.csv`; // Location of file containing two-way similar strings (output) 
/* END SETTINGS */
...
var brands=fs.readFileSync(FILE_CANONICAL,'utf8').split(/[\r\n]+/);
var brands_validated=fs.readFileSync(FILE_CANONICAL_VALIDATED,'utf8').split(/[\r\n]+/);

fs.writeFileSync(FILE_SIMILAR_ONEWAY,'');
...
for(let i=0;i<brands.length;i++){
	...
	for(let j=i+1;j<brands_validated.length;j++){
	...
	if(similar_oneway.length>0) fs.appendFileSync(FILE_SIMILAR_ONEWAY,similar_oneway.join('\n')+'\n');
}