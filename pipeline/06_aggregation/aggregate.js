...
const FILE_VALIDATED_CSV=`${FOLDER_VALIDATED}/validated.csv`;
const FOLDER_SIMILARITYCLUSTERS=`${FOLDER_DATASETS}/05_similarity_clusters`;
const FILE_SIMILARITY_ONEWAY_TSV=`${FOLDER_SIMILARITYCLUSTERS}/similar_oneway.csv`;
const FOLDER_AGGREGATED=`${FOLDER_DATASETS}/06_aggregated`;

const brandsPath = FILE_CANONICAL_CSV;
const validatedPath = FILE_VALIDATED_CSV;
const similarPath = FILE_SIMILARITY_ONEWAY_TSV;
const outPath = `${FOLDER_AGGREGATED}/aggregated_${threshold.toString().replace('0.','')}.json`;
const fs = require('fs');
